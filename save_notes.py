...
class SaveNotes(QMainWindow):
	...
	def eventFilter(self,source,event):
		if event.type() == QEvent.ContextMenu and source is self.notes_view:
			menu = QMenu()
			push = menu.addAction("Push to Lyse")
			copy = menu.addAction("Copy Notes to Editor")
			modify = menu.addAction("Modify Notes")
			if menu.exec_(event.globalPos()):
				item = source.itemAt(event.pos())
				# print(item.data(1))
				copy.triggered.connect(partial(self.copyNoteToEditor, self.NOTES[item.data(1)]))
				print(item.text())
			# push.triggered.connect(partial())
			# modify.triggered.connect()
			# copy.triggered.connect(partial(self.copyNoteToEditor, self.NOTES[item.data(1)]))
			# modify.triggered.connect()
